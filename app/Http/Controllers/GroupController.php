...
class GroupController extends Controller
{
    public function store(Request $request)
    {
        ...
        if(!empty($request->input('checkVal')))
        {
        ...
        return [
            'groupdata' => view('portal.chat.layouts.chat-group-list')->with(['groupdata' => $groupdata])->render()
        ];
    }
    ...
    public function getGroupMessage($group_id)
    {
        ...
        $userdata = DB::table('group_users')
                    ...
                    ->select('group_users.user_id', 'users.id', 'users.name')
                    ->where('group_users.group_id', '=', $group_id)->get();

        return [
            'view1' => view('portal.chat.layouts.group-message-data')->with(['messages' => $messages])->with(['groupdata' => $groupdata])->with(['userdata' => $userdata])->render(),
            'view2' => view('portal.chat.layouts.group-profile-details')->with(['messages' => $messages])->with(['groupdata' => $groupdata])->with(['userdata' => $userdata])->render()
        ];
    }
    ...
    public function getGroupLastMessage($group_id)
    {
        ...
        // Get all Group User data
        $userdata = DB::table('group_users')->join('users', 'group_users.user_id', '=', 'users.id')->select('group_users.user_id', 'users.id', 'users.name')
            ->where('group_users.group_id', '=', $group_id)->get();
        return view('portal.chat.layouts.group-conversation')->with(['messages' => $messages])->with(['userdata' => $userdata]);
    }
    ...
    public function groupsearch(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($request->search)
            {
                $groupdata = $groupdata->where('group_name' , 'LIKE', '%' . $request->search . '%');
            }

            $groupdata = $groupdata->get();

            $htmldata = view('portal.chat.layouts.chat-group-list')->with('groupdata', $groupdata)->render();
    ...
    public function groupmessagesearch(Request $request)
    {
        if ($request->ajax()) {
            ...
            $htmldata = view('portal.chat.layouts.group-conversation')->with(['messages' => $messages])->with(['userdata' => $userdata])->render();
            return Response($htmldata);
        }
    }
}
