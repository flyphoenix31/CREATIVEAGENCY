...
class ChatController extends Controller
{
    ...
    public function index()
    {
        ...
        $recentusers = Message::with('user')->latest()->distinct('to_user')->myChat()->get();


        $recentusers = Message::select(DB::raw('*, max(created_at) as updatedAt'))
                ->with('touser')
        ...
        return view('chat')->with(['page' => 'index', 'users' => $users, 'contacts' => $contacts, 'groupdata' => $groupdata, 'AttachedFiles' => $AttachedFiles]);

        return view('chat\index')->with(['users' => $users, 'contacts' => $contacts, 'groupdata' => $groupdata, 'AttachedFiles' => $AttachedFiles]);
    }
    ...
    public function getMessage($user_id)
    {
        ...
        return [
            'view1' => view('portal.chat.layouts.chat-message-data')->with(['messages' => $messages])->with(['chatUser' => $chatUser])->render(),
            'view2' => view('portal.chat.layouts.user-profile-details')->with(['messages' => $messages])->with(['chatUser' => $chatUser])->render()
        ];
    }
    ...
    public function getLastMessage($user_id)
    {
        ...
        return view('portal.chat.layouts.message-conversation')->with(['messages' => $messages])->with(['chatUser' => $chatUser]);
    }
