...
function selectableDropdown(getElement, myCallback) {
  ...
  for (var i = 0; i < getDropdownElement.length; i++) {
      getDropdownElement[i].addEventListener('click', function() {
        ...
        var dataSign = this.getAttribute('data-sign');
        ...
        var dropdownValues = {dropdownValue:dataValue, dropdownImage:dataImage, dropdowndataId:dataId, dropdataSign:dataSign};
...
function ChangeCurrency(value) {
    ...
    $('.currency_symbol').html(value.dropdataSign);

    $('#currency_id').val(value.dropdowndataId);

    $('#currency_sign').val(value.dropdataSign);

}

function updateCurrency() {
    $('.currency_symbol').html($('#currency_sign').val());
...
function additems()
{
    ...
    var currency = $('#currency_sign').val();
    ...
    '<td class="delete-item-row">'+
     ...
     ' </td>'+
      ...
      '<td class="text-right amount"><span class="editable-amount"><span class="currency_symbol">'+currency+'</span> <span class="item_total_'+row_id+'">0.00</span></td>'+
...
function additmeswithdata(key, value)
{
    var row_id = $('#childtypetr tr:last').attr('id');
    var istax = '';
	//var row_id = parseInt(row_id) || 0;
    ...
    '<td class="delete-item-row">'+
        ...
        '</ul>'+
      '</td>'+
      '<td class="description"><input type="text" class="form-control  form-control-sm" placeholder="Item Description" id="description.'+row_id+'" name="description[]" value="'+value.description+'"> <textarea class="form-control" placeholder="Additional Details" id="item_notes.'+row_id+'" name="item_notes[]">'+value.item_notes+'</textarea></td>'+
      '<td class="rate">'+
          '<input type="text" data-id="unit_price_'+row_id+'" class="unit_price form-control  form-control-sm" name="unit_price[]" id="unit_price.'+row_id+'" placeholder="Price" onkeyup="return calculatesubtotal('+row_id+')" value="'+value.unit_price+'">'+
     ' </td>'+
      '<td class="text-right qty"><input type="text" data-id="quantity_'+row_id+'" class="quantity form-control  form-control-sm" placeholder="Quantity" name="quantity[]" id="quantity.'+row_id+'" onkeyup="return calculatesubtotal('+row_id+')" value="'+value.quantity+'"></td>'+
      '<td class="text-right amount"><span class="editable-amount"><span class="currency_symbol">'+currency+'</span> <span class="item_total_'+row_id+'">0.00</span></td>'+
      '<td class="text-center tax taxavial">'+
          '<div class="n-chk">'+
              '<label class="new-control new-checkbox new-checkbox-text checkbox-primary" style="height: 18px; margin: 0 auto;">'+
                '<input '+istax+' type="checkbox" class="new-control-input taxavial" id="is_tax_avil_'+row_id+'" name="is_tax_avil[]" value="1">'+
                '<span class="new-control-indicator taxavial"></span><span class="new-chk-content">Tax</span>'+
             ' </label>'+
          '</div>'+
      '</td>'+
...
deleteItemRow();
selectableDropdown(document.querySelectorAll('.invoice-select .dropdown-item'), ChangeCurrency);
